#!/usr/bin/env python3
"""
Load FL 2024 historical election seed data into SQLite.

Usage:
    python scripts/seed_2024.py --db data/ballot-guide.db
    python scripts/seed_2024.py --db data/ballot-guide.db --replace
    python scripts/seed_2024.py --db data/ballot-guide.db --dry-run
"""
import argparse
import asyncio
import json
...
def load_seed_files() -> tuple[dict, list, list]:
    """Load and validate the three FL 2024 seed JSON files."""
    files = {
        "fl_2024_election.json": None,
        "fl_2024_measures.json": None,
        "fl_2024_candidates.json": None,
    }
    ...
    election_data = files["fl_2024_election.json"]
    measures = files["fl_2024_measures.json"]
    candidates = files["fl_2024_candidates.json"]

    if not isinstance(election_data, dict) or "election" not in election_data:
        raise ValueError("fl_2024_election.json must be an object with an 'election' key")
    if not isinstance(measures, list):
        raise ValueError("fl_2024_measures.json must be a JSON array")
    if not isinstance(candidates, list):
        raise ValueError("fl_2024_candidates.json must be a JSON array")
...
def validate_seed_data(
    ...
    """Validate seed data. Returns list of errors (empty = valid)."""
    errors: list[str] = []
    ...
    for i, candidate in enumerate(candidates):
        ...
        if candidate.get("positions_json") is not None:
            try:
                ...
                errors.append(f"Candidate[{i}] has invalid positions_json")
        if candidate.get("funding_summary_json") is not None:
            try:
                json.loads(candidate["funding_summary_json"])
            except (json.JSONDecodeError, TypeError):
                errors.append(f"Candidate[{i}] has invalid funding_summary_json")
...
async def seed_database(db_path: str, replace: bool = False) -> dict:
    """Insert FL 2024 seed data. Returns counts dict."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Seed FL 2024 historical election data")
    parser.add_argument("--db", required=True, help="Path to SQLite database file")
    parser.add_argument("--replace", action="store_true", help="Delete and re-insert FL 2024 data")
